...
public class BookDAOImpl implements BookDAO {
	...
	public int insertBook(Map<String, Object> book) {
		...
		try {
			con = Connector.open();
			String sql = "insert into book(b_num,b_title,b_autor,b_credat,b_desc)";
			sql += " values(seq_book_b_num.nextval, ?,?,sysdate,?)";
			ps = con.prepareStatement(sql);
			ps.setString(1, book.get("b_title").toString());
			ps.setString(2, book.get("b_autor").toString());
	...
	public List<Map<String, Object>> selectBookList(Map<String, Object> book) {
		...
		try {
			con = Connector.open();
			String sql="select b_num,b_title,b_autor,b_credat,b_desc from book";
			ps=con.prepareStatement(sql);
			rs=ps.executeQuery();
			while(rs.next()) {
				...
				map.put("b_autor",rs.getString("b_autor"));
	...
	public Map<String, Object> selectBook(int bNum) {
		...
		try {
			con = Connector.open();
			String sql="select b_num,b_title,b_autor,b_credat,b_desc from book where b_num=?";
			ps=con.prepareStatement(sql);
			ps.setInt(1,bNum);
			rs=ps.executeQuery();
	...
	public static void main(String[] args) {
		...
		//System.out.println(bdao.selectBook(24));
		//int result=bdao.deleteBook(24);
