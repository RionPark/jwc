...
public class MemberDAOImpl {
	...
	public int insertMember(Map<String, Object> member) {
		...
		try {
			...
			ps.setInt(1, (int)member.get("m_num"));
			ps.setNString(2, member.get("b_name").toString());
			ps.setNString(2, member.get("b_id").toString());
			ps.setNString(2, member.get("b_pwd").toString());
			ps.setNString(3, member.get("m_credat").toString());
	...
	public int updateMember(Map<String, Object> member) {
		...
		try {
			...
			ps.setInt(1, (int)member.get("m_num"));
			ps.setString(2, member.get("m_name").toString());
			ps.setString(3, member.get("m_id").toString());
			ps.setString(4, member.get("m_pwd").toString());
			ps.setString(5, member.get("m_credat").toString());
	...
	public List<Map<String, Object>> selectMemberList(Map<String, Object> member) {
		...
		try {
			con = Connector.open();
			String sql = "select m_num, m_title, m_author, m_credat, m_desc from member";
			ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
			while (rs.next()) {
	...
	public static void main(String[] args) {
		MemberDAOImpl bdao = new MemberDAOImpl();
		Map<String, Object> map = new HashMap<>();
		map.put("m_name", "개");
		map.put("m_id", "dog");
		map.put("m_pwd", "dog123");

map.put("m_num",1);
int result = bdao.updateMember(map);
